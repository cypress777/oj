...
int main() {
    ...
    for (int i = 0; i < Q; i++) {
        ...
        if (c_max >= c_min && horizontal_tree[max(r1, r2)].GetMax(1, 0, M - 1, c_min, c_max) >= abs(r2 - r1) - 1 ||
            r_max >= r_min && vertical_tree[max(c1, c2)].GetMax(1, 0, N - 1, r_min, r_max) >= abs(c2 - c1) - 1) {
            cout << 2 << endl;
            continue;
        }
        cout << -1 << endl;
